...
GO4FEET:
	...
	LOAD	XPOS			; STORES THE X COORDINATE BEFORE RESET
	STORE	RESETX
	LOAD	YPOS			; STORES THE Y COORDINATE BEFORE RESET
	STORE	RESETY
	OUT		RESETPOS
...
SEARCH0:
	...
	SUB		TWOFEET
	SUB		TWOFEET
	JNEG	FORWARD			; IF THERE IS NOTHING 4 FEET AWAY, MORE FORWARD
	LOAD	TEMPDIST		; ELSE, STORE IN SENSOR0 AND BEEP
	STORE	SENSOR0
	JUMP	FINDSMALL
	;OUT		BEEP		
...
; 	JPOS	CHOSE5
	...
	JZERO	ROTATELEFT		
	LOAD	XPOS			; STORE THE CURRENT X POSITION OF THE ROBOT
	STORE	X
	LOAD	YPOS			; STORE THE CURRENT Y POSITION OF THE ROBOT
	STORE	Y
...
TOWARDSOBJECT:				; MOVE FORWARD UNTIL YOU GET TO THE OBJECT
	LOAD	FMid			; MOVE FORWARD
	OUT    	LVELCMD     
	OUT    	RVELCMD	
	LOAD	CURRENTSENSOR	; STORE THE DISTANCE OF CLOSEST OBJECT TO TEMPVAR
	STORE 	TEMPVAR
	LOAD	XPOS			; L2X <- (XPOS-X) 
	SUB		X				
	STORE	L2X
	LOAD	YPOS			; L2Y <- (YPOS-Y)
	SUB		Y
	STORE	L2Y
	CALL	L2Estimate		; L2Estimate = SQRT((XPOS-X)^2+(YPOS-Y)^2)
	SUB		TEMPVAR			
	JNEG	TOWARDSOBJECT	; IF ROBOT HAS NOT TRAVELED FAR ENOUGH, REPEAT
	JUMP	NOISE			; IF ROBOT HAS REACHED THE OBJECT (HOPEFULLY), BEEP
	
NOISE:
	OUT		BEEP	
			
ROTATERIGHT:				; ROTATES CORRECT DEGREES TO THE RIGHT
	LOAD	FMid
	OUT 	LVELCMD
	IN 		THETA
	ADD		CURRENTANGLE
...
;*********************************************************************;
				ORG 	&H350
CURRENTSENSOR:	DW		&HFFFF		; CURRENT SENSOR USED FOR CALCULATIONS
CURRENTANGLE:	DW		&HFFFF		; CURRENT ANGLE OF THE SENSOR
